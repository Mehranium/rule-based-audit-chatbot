...
# =====================================================================================
# Merge helpers (robust against presence/absence of target columns in the left frame)
# =====================================================================================
def coalesce_columns(df: pd.DataFrame, preferred: str, fallback: str, out: str = None):
    ...
    if preferred in df.columns:
        # Use preferred where it is non-null and non-empty after strip
        lhs = df[preferred]
        ...
        # No preferred -> just use fallback
        df[target] = df[fallback] if fallback in df.columns else pd.Series([None]*len(df))
    return df

# =====================================================================================
# Coalesce Company & Year into Finding from Main (ALWAYS, without suffix fragile logic)
# =====================================================================================
...
# =====================================================================================
# Coalesce Company & Audit Year into Introduction from Main (ALWAYS)
# =====================================================================================
intro = intro.merge(
    main[['Report File Name', 'Company', 'Audit Year']].rename(
        columns={'Company': 'Company_from_main', 'Audit Year': 'AuditYear_from_main'}
...
# Keep Audit Year column available for some outputs; if missing, set from helper
    ...
    for _, (pattern, tag, qtype) in enumerate(patterns, 1):
        ...
        if qtype == 'main':
            if tag == 'main_companies_in_country':
                ...
                return f"Companies located in {country}:\n" + "\n".join(f"- {c}" for c in sorted(companies))
                ...
                return f"KF responsible for {company}:\n" + "\n".join(f"- {x}" for x in sorted(kfs))
                ...
                return f"EVP/SVP responsible for {company}:\n" + "\n".join(f"- {x}" for x in sorted(evps))
                ...
                return f"Auditors for {company} in {year}:\n" + "\n".join(f"- {n}" for n in names)
                ...
                return f"Companies with {person} as KF:\n" + "\n".join(f"- {c}" for c in sorted(companies))
                ...
                return f"Companies with {person} as EVP/SVP:\n" + "\n".join(f"- {c}" for c in sorted(companies))
                ...
                return f"Companies audited by {auditor} in {year}:\n" + "\n".join(f"- {c}" for c in sorted(companies))
        ...
        if qtype in ('company_keyword_year', 'company_keyword_year_range') and tag == 'composite':
            show_type = match.group(1).strip().lower()       # findings | risks | recommendations
            company_raw = match.group(2).strip()
            keyword = match.group(3).strip()
            field_raw = match.group(4).strip().lower()       # finding category | finding title | finding | risk | recommendation
            ...
            else:  # company_keyword_year_range
        ...
        if qtype == 'intro':
            if tag == 'intro_dates':
                company = match.group(1).strip()
                year = int(match.group(2))
                mask = (intro_df['Company'].fillna('').map(canon).str.contains(canon(company), na=False)) & \
                       (pd.to_datetime(intro_df['Audit Start Date'], errors='coerce').dt.year == year)
                ...
                mask = (pd.to_datetime(intro_df['Audit Start Date'], errors='coerce') >= date1) & \
                       (pd.to_datetime(intro_df['Audit Start Date'], errors='coerce') <= date2)
                rows = intro_df.loc[mask]
                ...
                if pd.isna(date1) or pd.isna(date2):
                    return "Invalid date format. Please use dd.mm.yyyy."
                mask = (pd.to_datetime(intro_df['Audit End Date'], errors='coerce') >= date1) & \
                       (pd.to_datetime(intro_df['Audit End Date'], errors='coerce') <= date2)
                rows = intro_df.loc[mask]
                companies = rows['Company'].dropna().unique()
                if not companies.size:
                ...
                mask = (pd.to_numeric(intro_df['Audit Duration (Day)'], errors='coerce') < n) & \
                       (pd.to_datetime(intro_df['Audit Start Date'], errors='coerce').dt.year.between(y1, y2))
        ...
        # ============================
        if qtype == 'suggestion':
            sug = suggestion_df.merge(
                main_df[['Report File Name', 'Company', 'Audit Year']],
                on='Report File Name',
                how='left'
            )
            ...
            if tag == 'suggestion_issue_titles_company_year':
                ...
                lines = []
                for _, r in rows.iterrows():
                    lines.append(f"- **Issue Title:** {r.get('Issue Title', '')}\n  **Issue:** {r.get('Issue', '')}\n  **Suggestion:** {r.get('Suggestion', '')}")
                ...
                def _fmt(ittl, iss, sgg, yr):
                    return f"- {ittl or ''} ({int(yr)}):\n  Issue: {iss or ''}\n  Suggestion: {sgg or ''}"
                body = "\n".join(_fmt(ittl, iss, sgg, yr) for ittl, iss, sgg, yr in rows.itertuples(index=False, name=None))
                return f"{header}\n{body}"
...
# --- CLI chatbot loop ---
def chatbot():
    print("Welcome to the Audit Chatbot! Ask your question (type 'exit' to quit).")
    while True:
